...
func spawn_object2():  
	if get_child_count() < max_objects:  
		var object2_scene = preload("res://Prefabs/object1.tscn")  
		var object2 = object2_scene.instantiate()  

		# Tenta encontrar uma posição válida para o objeto  
		var valid_position_found = false  
		var attempts = 0  
		while not valid_position_found and attempts < 10:  
			object2.position.x = randf_range(0, 208)  # Garantir que a posição X esteja entre 0 e 208
			object2.position.y = -object_height  # Posição Y acima da tela  
			
			valid_position_found = true  
			
			# Verifica se a nova posição não colide com outros objetos
			for child in get_children():  
				if child is Node2D and child != object2:  
					if abs(child.position.x - object2.position.x) < min_distance:  
						valid_position_found = false  # Se estiver muito perto, tenta novamente  
						break  
			attempts += 1  # Aumenta a contagem de tentativas  
		
		if valid_position_found:  
			add_child(object2)  # Adiciona o objeto se a posição for válida  
...
func _on_timer_timeout() -> void:  
	spawn_object2()  # Chama a função para gerar um novo objeto  
